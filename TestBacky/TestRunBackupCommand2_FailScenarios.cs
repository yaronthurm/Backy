...
namespace TestBacky
{
    ...
    public class TestRunBackupCommand2_FailScenarios
    {
        ...
        [TestMethod]
        public void Backup_04_1_Running_for_the_second_time_Only_renamed_files_Failing_to_rename_some_files()
        {
            // This test simulates running the tool for the second time after some files were renamed
            // while having errors to rename some of the file.
            // The next time the backup is run will make sure all the files are renamed correctly
            ...
            {
                if (currentName.Contains("file1.txt"))
                    throw new Exception($"Failed to rename {currentName} to {newName}");
            };
            cmd.Execute();
            cmd.Failures.Count.ShouldBe(1);
